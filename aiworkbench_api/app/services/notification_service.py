...
DOMAIN_SCOPED_GOVERNOR_ROLES = ("ai_leader", "domain_owner")


def governor_user_ids(db: Session, domain_id: str | None) -> list[str]:
    """Governors who can act on this domain: portal admins, domain owner, and scoped ai_leaders/domain_owners.

    ai_leader and domain_owner receive assign notifications only when they have DomainAccess
    (or are listed as owner_id). Portal admins are always included.
    """
    ids: list[str] = [
    ...
    if domain_id:
        ...
        for role_name in DOMAIN_SCOPED_GOVERNOR_ROLES:
            for user in get_active_users_by_role_names(db, (role_name,)):
...
def _is_current_governor(
    ...
    if domain.owner_id == user.user_id:
        return True
    if domain.domain_id not in accessible_domain_ids:
        return False
    return role_name in ("ai_leader", "domain_owner")
...
def _use_case_notification_is_visible(
    ...
    if (
        ...
        and not {"initiate_assessment", "contribute_assessment"}.intersection(permissions)
    ):
        return False
