...
from app.core.ai_user_messages import (
...
from app.services.llm.adapters import rate_limit_user_message
from app.services.use_case_assessment_service import save_response

_ASSESSMENT_PREFILL_SYSTEM_PROMPT = (
    "Return a JSON object only. Do not wrap it in markdown fences or add extra text. "
    "The object must include items (template_item_id, selected_labels, comment) and optional overall_findings. "
...
def _build_use_case_documentation_context(db: Session, use_case: UseCase) -> str:
    ...
    if doc_quality:
        ...
        if doc_quality.analysis_payload:
            sections.append(f"Analysis detail: {doc_quality.analysis_payload[:800]}")
    else:
        sections.append("[not documented]")
...
def _question_batch(snapshot: dict, batch_size: int = 6) -> list[list[dict]]:
    questions: list[dict] = []
        ...
        for batch_index, batch in enumerate(batches):
            ...
            try:
                ...
                if exc.status_code == 429:
                    raise ValueError(rate_limit_user_message(exc.detail)) from exc
                detail = (exc.detail or "").lower()
                if "content filter" in detail:
                    raise ValueError(AI_CONTENT_FILTER_MESSAGE) from exc
                raise ValueError(AI_SERVICES_UNAVAILABLE_MESSAGE) from exc
            except LLMStructuredOutputError as exc:
                logger.error("Assessment AI prefill failed: %s", exc)
                raise ValueError(AI_STRUCTURED_OUTPUT_FAILED_MESSAGE) from exc
            except LLMProviderTimeoutError as exc:
                logger.error("Assessment AI prefill failed: %s", exc)
                raise ValueError(AI_PROVIDER_TIMEOUT_MESSAGE) from exc
            except LLMProviderConnectionError as exc:
                logger.error("Assessment AI prefill failed: %s", exc)
                raise ValueError(AI_CONNECTION_DROPPED_MESSAGE) from exc
