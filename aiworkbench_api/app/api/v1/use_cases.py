...
def _is_workflow_governor(db: Session, user: User, domain_id: str) -> bool:
    """portal_admin, domain owner, or ai_leader with access to this domain — not track assignees."""
    if is_admin(db, user):
        return True
    if is_domain_owner(db, user, domain_id):
        return True
    if get_user_role_name(db, user) == "ai_leader":
        return _user_can_open_domain(db, user.user_id, domain_id)
    return False
