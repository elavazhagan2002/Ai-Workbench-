...
from app.core.authorization import check_permission, get_user_with_permissions, is_portal_admin
...
def _has_initiate_assessment_permission(db: Session, user: User) -> bool:
    return check_permission(db, user, "initiate_assessment", allow_admin=True)
...
def _is_assessment_owner_or_admin(db: Session, user: User, use_case: UseCase) -> bool:
    if is_portal_admin(db, user):
        return True
    return bool(use_case.assessment_owner and use_case.assessment_owner == user.user_id)


def _require_assessment_owner_or_admin(db: Session, user: User, use_case: UseCase) -> None:
    if _is_assessment_owner_or_admin(db, user, use_case):
        return
    raise HTTPException(
        status_code=status.HTTP_403_FORBIDDEN,
        detail="Only the assigned assessment owner or portal_admin can manage this assessment",
    )


def _require_initiate_assessment_permission(db: Session, user_id: str) -> User:
...
def _require_assessment_update_permission(
    ...
    *,
    ...
    """Match UI: only assessment owner/admin may edit; contribute covers owner without initiate."""
    user, _, _ = get_user_with_permissions(db, user_id)
    _require_assessment_owner_or_admin(db, user, use_case)
...
async def create_or_get_assessment(
    ...
    use_case = _load_use_case(db, use_case_id)
    _require_use_case_access(db, user_id, use_case)
    user = _require_initiate_assessment_permission(db, user_id)
