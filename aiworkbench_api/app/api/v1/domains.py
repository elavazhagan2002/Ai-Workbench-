...
def _ensure_domain_access(
    db: Session,
    domain_id: str,
    user_id: str,
    acting_user_id: str,
) -> None:
    """Grant DomainAccess when missing so owners and assignees can open the domain."""
    if not user_id:
        return
    existing = (
...
async def update_domain(
    ...
    if owner_id_new is not None:
        ...
        owner_changed = db_domain.owner_id != owner_id_new
        db_domain.owner_id = owner_id_new
        _ensure_domain_access(db, domain_id, owner_id_new, user_id)
    ...
    if owner_changed and owner_id_new:
        new_owner = db.query(User).filter(User.user_id == owner_id_new).first()
        previous_owner = (
            db.query(User).filter(User.user_id == old_owner_id).first() if old_owner_id else None
        )
        notify_domain_owner_changed(
            db,
            db_domain,
...
async def update_domain_owner(
    ...
    _ensure_domain_access(db, domain_id, owner_update.owner_id, user_id)
