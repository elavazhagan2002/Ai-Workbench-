...
class User(Base):
    ...
    totp_prompt_seen = Column(Boolean, default=False, nullable=False)
    totp_confirmed_at = Column(DateTime, nullable=True)
    preferred_mfa_method = Column(String(20), default="email", nullable=False)
...
class UseCase(Base):
    ...
    deployment_model = Column(String(40), nullable=True)
...
class DocumentType(Base):
    """Lookup table for use-case document types (admin-managed)."""

    __tablename__ = "document_types"

    doc_type_id = Column(String(36), primary_key=True, default=generate_uuid)
...
class Notification(Base):
    """Per-user in-app notification inbox rows."""

    __tablename__ = "notifications"

    notification_id = Column(String(36), primary_key=True, default=generate_uuid)
    recipient_user_id = Column(String(36), ForeignKey("users.user_id", ondelete="CASCADE"), nullable=False, index=True)
    actor_user_id = Column(String(36), ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True)
    ...
    actions = Column(JSON, nullable=True)  # e.g. ["open", "reject_analysis"]
    payload = Column(JSON, nullable=True)
    is_read = Column(Boolean, default=False, nullable=False)
    read_dt = Column(DateTime, nullable=True)
    dismissed_dt = Column(DateTime, nullable=True)
    created_dt = Column(DateTime, server_default=func.now(), nullable=False)
