...
interface AuthContextType {
  ...
  completePendingLogin: (userData: User) => void;
  signOut: () => Promise<void>;
  updateProfile: (updates: Partial<User>) => Promise<void>;
  applyUser: (userData: User) => void;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const refreshCurrentUser = useCallback(async () => {
    if (refreshInFlightRef.current || !sessionStorage.getItem('userId')) return;
    ...
    try {
      ...
      if (error?.message && !error.message.includes('401') && !error.message.includes('Unauthorized')) {
        logger.warn('Unable to refresh current permissions', error);
      }
    } finally {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(refreshIfVisible, USER_REFRESH_INTERVAL_MS);
  ...
  }, [user?.user_id, refreshCurrentUser]);
    ...
    try {
      ...
      if (userStr) {
        try {
          ...
          if (lastLoginTime && (now - Number.parseInt(lastLoginTime, 10)) < oneDayInMs) {
            setUser(userData);
            setLoading(false);

            const lastVerifiedAt = sessionStorage.getItem('userVerifiedAt');
            const recentlyVerified =
              lastVerifiedAt && now - Number.parseInt(lastVerifiedAt, 10) < USER_VERIFY_TTL_MS;
  ...
  async function signIn(email: string, password: string) {
    ...
    if (response.status === 'LOGIN_SUCCESS' && !response.totp_setup_suggested) {
  ...
  function completePendingLogin(userData: User) {
    commitLoginSuccess(userData);
  }

  function applyUser(userData: User) {
    sessionStorage.setItem('userId', userData.user_id);
    sessionStorage.setItem('user', JSON.stringify(userData));
    sessionStorage.setItem('userVerifiedAt', Date.now().toString());
    setUser(userData);
  ...
  function hasPermission(permission: string): boolean {
    if (user?.role?.role_name === 'portal_admin') return true;
    return user?.permissions?.includes(permission) || false;
  }

  return (
    <AuthContext.Provider value={{ user, loading, signIn, verifyLoginPasscode, completePendingLogin, signOut, updateProfile, applyUser, refreshCurrentUser, hasPermission, clearSession }}>
      {children}
