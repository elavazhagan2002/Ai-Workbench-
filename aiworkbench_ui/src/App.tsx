...
function detailTabForSection(section: string | null): UseCaseDetailTab {
  if (section === 'tech_analysis' || section === 'analysis') return 'technical';
  if (section === 'business_analysis') return 'business';
  if (section === 'estimate' || section === 'roi' || section === 'data' || section === 'risks' || section === 'comments' || section === 'assessment') {
    return section;
  }
  return 'overview';
}
...
function AppContent() {
  ...
  const [selectedUseCaseDetailTab, setSelectedUseCaseDetailTab] = useState<UseCaseDetailTab>('overview');
  ...
  function handleNotificationLink(link: string) {
    ...
    if (parsed.useCaseId && parsed.page === 'usecase-detail') {
      if (parsed.domainId) setSelectedDomainId(parsed.domainId);
      handleViewUseCase(parsed.useCaseId, detailTabForSection(parsed.section));
      return;
    }
    if (parsed.useCaseId && parsed.page === 'usecase-edit') {
