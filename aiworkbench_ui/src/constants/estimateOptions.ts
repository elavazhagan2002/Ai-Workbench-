...
export function computeEstimateInvestment(data: EstimateData | null | undefined): number {
  if (!data) return 0;
  const totals = computeEstimateTotals(data.lines);
  return Math.round((totals.year1 + totals.year2 + totals.year3) * 100) / 100;
}

export const MAX_ESTIMATE_AMOUNT = 999_999_999_999.99;

export type MoneyField = 'year1' | 'year2' | 'year3';
...
export function validateMoneyValue(value: number | null, required: boolean, label = 'This amount'): string | null {
...
export function lineFieldErrors(
  ...
  for (const field of ['year1', 'year2', 'year3'] as const) {
    const err = validateMoneyValue(
      line[field],
      required,
      `${ESTIMATE_LINE_LABELS[key]} ${MONEY_FIELD_LABELS[field]}`
    );
...
export function lineYearTotal(line: EstimateLine): number {
  return [line.year1, line.year2, line.year3].reduce<number>(
...
export function lineNavState(
  ...
  if (invalidMessages.length) {
    return {
      tone: 'error',
      hint: invalidMessages[0],
      issueCount: invalidMessages.length,
    };
  }
  ...
  const amountMissing = (['year1', 'year2', 'year3'] as const).filter((field) => missing[field]);
  if (amountMissing.length === 3) parts.push('Enter Year 1–3');
  ...
  if (showWarnings) {
    return { tone: 'warn', hint, issueCount: missingCount };
  }
  return {
    tone: 'idle',
...
export function estimateFilledValueErrors(data: EstimateData): string | null {
  ...
  for (const key of ESTIMATE_LINE_KEYS) {
    ...
    if (!first) continue;
    if (errors.type && first === errors.type) return `${ESTIMATE_LINE_LABELS[key]}: ${first}`;
    return first;
  }
