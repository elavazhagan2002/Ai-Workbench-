...
export default function AssessmentChecklistSettings() {
  ...
  const completeQuestionCount = form.areas.reduce(
    (count, area) => count + area.items.filter((item) => isQuestionComplete(item)).length,
    0,
  );
  const canActivateTemplate =
    ...
    !!selectedTemplateId &&
    !isActiveTemplate &&
    ['DRAFT', 'IN REVIEW'].includes(form.status) &&
    questionCount > 0 &&
    completeQuestionCount === questionCount;
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {editorMode !== 'list' && canActivateTemplate && (
              <button
                type="button"
                ...
                title="Make this the active effective checklist used by AI Assessment"
              >
                    ...
                    <tr key={template.template_id}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-2">
                          <HoverTip label="View template" side="top">
                            <button type="button" onClick={() => openTemplate(template.template_id, 'view')} aria-label="View template" className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700">
                              <Eye className="h-4 w-4 text-slate-600 hover:text-blue-600 dark:text-slate-400" />
                            </button>
                          </HoverTip>
                          {!template.is_active && ['DRAFT', 'IN REVIEW'].includes(template.status) && (
                            <HoverTip label="Edit template" side="top">
                              <button type="button" onClick={() => openTemplate(template.template_id, 'edit')} aria-label="Edit template" className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700">
                                <Edit2 className="h-4 w-4 text-slate-600 hover:text-blue-600 dark:text-slate-400" />
                              </button>
                            </HoverTip>
                          )}
                          <HoverTip label="Clone template" side="top">
                            <button type="button" onClick={() => startClone(template)} aria-label="Clone template" className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700">
                              <Copy className="h-4 w-4 text-slate-600 hover:text-blue-600 dark:text-slate-400" />
                            </button>
                          </HoverTip>
                          {template.is_active && (
                            <HoverTip label="Deprecate active template" side="top">
                              <button type="button" onClick={() => setDeprecateTarget(template)} aria-label="Deprecate active template" className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700">
                                <Archive className="h-4 w-4 text-slate-600 hover:text-rose-600 dark:text-slate-400" />
                              </button>
                            </HoverTip>
                          )}
                          {!template.is_active && ['DRAFT', 'IN REVIEW'].includes(template.status) && (
                            <HoverTip label="Activate (mark effective)" side="top">
                              <button type="button" onClick={() => handleActivate(template.template_id)} aria-label="Activate (mark effective)" className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700">
                                <CheckCircle2 className="h-4 w-4 text-slate-600 hover:text-emerald-600 dark:text-slate-400" />
                              </button>
                            </HoverTip>
                          )}
                          {!template.is_active && ['DRAFT', 'IN REVIEW'].includes(template.status) && (
                            <HoverTip label="Delete template" side="top">
                              <button type="button" onClick={() => setDeleteTarget(template)} aria-label="Delete template" className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700">
                                <Trash2 className="h-4 w-4 text-slate-600 hover:text-red-600 dark:text-slate-400" />
                              </button>
                            </HoverTip>
                          )}
                        </div>
        ...
        {editorMode !== 'list' && editorMode !== 'clone' && (
          <div className="space-y-4">
            <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
              <div className="grid gap-4 lg:grid-cols-3">
                ...
                {(editorMode === 'edit' || editorMode === 'view') && (
                  <div>
                    ...
                    />
                    {canEdit && !isActiveTemplate && (
                      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                        Draft templates cannot be used on use cases. Click Activate to publish this checklist as the active effective template.
                      </p>
                    )}
                  </div>
                )}
              ...
              {canEdit && !isActiveTemplate && questionCount > 0 && completeQuestionCount === questionCount && (
                <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-200">
                  This checklist is {form.status}. Activate it to make it the template used by AI Assessment on use cases.
                </div>
              )}
                ...
                {!activeQuestion ? (
                  ...
                  <>
                    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <HoverTip label="Previous question">
                          <button type="button" onClick={() => navigateQuestion(-1)} className="rounded-lg border border-slate-300 p-2 dark:border-slate-600" aria-label="Previous question">
                            <ChevronLeft className="h-4 w-4" />
                          </button>
                        </HoverTip>
                        <HoverTip label="Next question">
                          <button type="button" onClick={() => navigateQuestion(1)} className="rounded-lg border border-slate-300 p-2 dark:border-slate-600" aria-label="Next question">
                            <ChevronRight className="h-4 w-4" />
                          </button>
                        </HoverTip>
                        {canEdit && (
                          <HoverTip label={savingQuestion ? 'Saving question…' : 'Save this question'}>
                            <button
                              type="button"
                                  ...
                                  <tr key={`${activeQuestion.item.sno}-${answerIndex}`}>
                                    ...
                                    {canEdit && (
                                      <td className="px-3 py-2">
                                        <HoverTip label="Remove answer option" side="top" align="end">
                                          ...
                                          }} className="text-red-600 hover:text-red-700" aria-label="Remove answer option">
