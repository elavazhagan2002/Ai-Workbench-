...
import UseCaseDocumentAttachments from './UseCaseDocumentAttachments';
...
function isBlankField(raw: unknown) {
  return raw == null || (typeof raw === 'string' && !raw.trim()) || (Array.isArray(raw) && raw.length === 0);
}

function trackHasProgress(track: 'technical' | 'business', formData: FormShape) {
  const required = track === 'technical' ? TECHNICAL_REQUIRED_FIELDS : BUSINESS_REQUIRED_FIELDS;
  return required.some((field) => !isBlankField((formData as Record<string, unknown>)[field.key]));
}

function trackSnapshot(track: 'technical' | 'business', formData: FormShape) {
  if (track === 'technical') {
    return JSON.stringify({
      ai_category: formData.ai_category,
      tool_complexity: formData.tool_complexity,
      host_system_capability: formData.host_system_capability,
      data_privacy_security: formData.data_privacy_security,
      solution_design_overview: formData.solution_design_overview,
      deployment_model: formData.deployment_model,
    });
  }
  return JSON.stringify({
    ...
    human_in_loop_strategy: formData.human_in_loop_strategy,
    bias_assessment_performed: formData.bias_assessment_performed,
    protected_attributes: formData.protected_attributes,
    balancing_strategy: formData.balancing_strategy,
    frequency_of_task: formData.frequency_of_task,
    current_effort: formData.current_effort,
    user_group_size: formData.user_group_size,
    efficiency_impact: formData.efficiency_impact,
    quality_compliance_impact: formData.quality_compliance_impact,
    user_urgency: formData.user_urgency,
    process_impact: formData.process_impact,
    operational_compliance_risk: formData.operational_compliance_risk,
  });
}

function analysisSurveyHint(opts: {
  completed: boolean;
  showStart: boolean;
  ...
  if (opts.completed) {
    return 'This track is marked completed. Answers are locked for every role.';
  }
  if (opts.showStart) {
    return 'Start this analysis to open the survey, or reject the assignment.';
  }
  if (!opts.canOpenEditor && !opts.started) {
    return 'Waiting for the assigned owner to start this analysis.';
  }
  if (opts.isEditing) {
    return opts.dirty
      ? 'You have unsaved changes. Save to validate required fields.'
      : 'Edit the survey. Save appears after you change an answer.';
  }
  if (opts.canOpenEditor && opts.allRequiredFilled) {
    return 'All required fields are saved. Complete this analysis when you are done, or click Edit to change answers.';
  }
  if (opts.canOpenEditor) {
    return 'Saved answers are view-only. Click Edit to finish remaining required fields.';
  }
  return 'View-only for your role on this track.';
}
...
export default function AnalysisPanels({
  ...
  const [started, setStarted] = useState(() => completed || trackHasProgress(track, formData));
  const [isEditing, setIsEditing] = useState(false);
  const [baseline, setBaseline] = useState(() => trackSnapshot(track, formData));
  const [dirty, setDirty] = useState(false);
  const locked = completed || isReadOnly || !canEditTrack || !isEditing || !started;
  ...
  useEffect(() => {
    if (completed || trackHasProgress(track, formData)) {
      setStarted(true);
    }
  }, [completed, formData, track]);
  ...
  useEffect(() => {
    ...
    setDirty(trackSnapshot(track, formData) !== baseline);
  }, [baseline, formData, isEditing, track]);

  const missingRequired = useMemo(() => {
    const required = track === 'technical' ? TECHNICAL_REQUIRED_FIELDS : BUSINESS_REQUIRED_FIELDS;
    const missing: { key: string; label: string }[] = [];
    for (const field of required) {
      if (isBlankField((formData as Record<string, unknown>)[field.key])) {
  ...
  const showForm = completed || started;
  const showStart = canOpenEditor && !started;
  const showSave = canOpenEditor && started && isEditing && dirty;
  const showEdit = canOpenEditor && started && !isEditing;
  const showComplete = canComplete && !completed && started && !isEditing && allRequiredFilled;

  const surveyHint = analysisSurveyHint({
    completed,
    showStart,
    canOpenEditor,
    started,
  ...
  function beginEdit() {
    setBaseline(trackSnapshot(track, formData));
    setDirty(false);
  ...
  async function handleSaveClick(e?: React.FormEvent) {
    ...
    const result = await onSave(syntheticEvent);
    if (result !== false) {
      setBaseline(trackSnapshot(track, formData));
      setDirty(false);
      setIsEditing(false);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          ...
          {showValidation && missingRequired.length > 0 && (
            <p className="mt-2 text-sm font-medium text-red-600 dark:text-red-400">
              Fill required fields before saving: {missingRequired.map((m) => m.label).join(', ')}.
            </p>
          )}
          {showComplete && completeBlockedReason && (
        ...
        <div className="flex flex-wrap gap-2">
          {showStart && (
            <button type="button" onClick={handleStartAnalysis} className="wb-btn-primary">
              <Play className="h-4 w-4" />
              Start Analysis
            </button>
          )}
          {showEdit && (
            <button type="button" onClick={beginEdit} className="wb-btn-secondary">
              <Pencil className="h-4 w-4" />
              Edit
            </button>
          )}
          {showSave && (
            <button type="button" onClick={() => handleSaveClick()} disabled={saving} className="wb-btn-primary">
        ...
        {track === 'technical' ? (
          <>
            ...
            <SurveyQuestion
              ...
              required
              fieldKey="solution_design_overview"
              invalid={showValidation && missingKeys.has('solution_design_overview')}
            >
              <AIFieldAssist
            ...
            <SurveyQuestion
              ...
              help="Where will this solution run in production?"
              required
              fieldKey="deployment_model"
              invalid={showValidation && missingKeys.has('deployment_model')}
            >
              <RadioOptionGroup
                ...
                disabled={disabled}
                columns={2}
                onChange={(v) => setFormData({ ...formData, deployment_model: v })}
              />
            </SurveyQuestion>
          </>
        ) : (
      ...
      {track === 'technical' && showForm && useCaseId && onDocumentsChange ? (
        <div className="wb-card-pad">
          <UseCaseDocumentAttachments
            useCaseId={useCaseId}
            documents={documents}
            onDocumentsChange={onDocumentsChange}
            canUpload={canOpenEditor}
            source="technical_analysis"
            filterSource="technical_analysis"
            title="Technical supporting files"
            compact
            helpText={
              canOpenEditor
                ? 'These files stay with Technical Analysis and are not shown in Resource management.'
                : 'Technical supporting files for this analysis. They are not part of Resource management.'
            }
          />
        </div>
      ) : null}
