          ...
          <div data-dropdown-boundary className="wb-scroll-slim min-h-0 flex-1 space-y-4 overflow-y-auto overflow-x-hidden p-5">
            ...
            {loading ? (
              ...
              <p className="text-sm text-amber-800 dark:text-amber-200 bg-amber-50 dark:bg-amber-950/30 rounded-lg px-3 py-2">
                No eligible owners have access to this domain. Grant domain access first, then assign.
              </p>
            ) : (
              <>
                <div>
...
export function RoiAssignModal(
  ...
  const subtitle = isReassign
    ...
    : 'Select who will complete the ROI savings worksheet. Only people with access to this domain are listed.';
  return (
    <SimpleAssignModal
      {...rest}
...
export function AssessmentAssignModal(
  ...
  const { domainId, onAssign, ...rest } = props;
  const loadOwners = useCallback(
    () => api.getEligibleAssessmentOwners(domainId),
    [domainId]
  );
  return (
    <SimpleAssignModal
      ...
      subtitle="Assign a governance-team member who has access to this domain to complete the Responsible AI assessment."
      loadOwners={loadOwners}
