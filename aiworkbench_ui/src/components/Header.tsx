    ...
    {createPortal(
      ...
      {showTotpModal && (
        <div className="wb-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="totp-modal-title">
          <div className="wb-modal max-w-2xl overflow-hidden p-0">
            <div className="flex items-start justify-between gap-3 border-b border-line px-5 py-4 sm:px-6">
              <div className="flex min-w-0 items-start gap-3">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-500 to-blue-600 text-white shadow-[0_8px_18px_rgba(14,165,233,0.22)]">
                  <Smartphone className="h-5 w-5" />
                </div>
                <div className="min-w-0">
                  <h3 id="totp-modal-title" className="font-display text-lg font-semibold text-ink">
                    {user?.totp_enabled ? 'Authenticator app' : 'Set up authenticator'}
                  </h3>
                  <p className="mt-0.5 text-sm text-ink-muted">
                    {user?.totp_enabled
                      ? 'A 6-digit app code is available when you sign in from an unknown network.'
                      : 'Scan once, then use Google Authenticator, Microsoft Authenticator, or Authy on unknown networks.'}
                  </p>
                </div>
              </div>
              <button
