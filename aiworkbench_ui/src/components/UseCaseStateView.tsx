...
interface UseCaseStateViewProps {
  ...
  /** At least one risk exists (open is allowed). Used to prompt before Estimate. */
  hasRisks?: boolean;
  /** Shown when the user clicks Estimate with no risks yet (do not open Assign). */
  onNeedRisks?: () => void;
...
export const ESTIMATE_CLICK_GATE =
  'Add at least one risk on the Risks tab before moving to Estimate. The assigned technical or business owner can add it (domain owner or portal admin can add it as backup). The risk may remain open until Approve.';
...
export default function UseCaseStateView({
  ...
  const [estimateClickError, setEstimateClickError] = useState('');

  useEffect(() => {
    if (hasRisks) setEstimateClickError('');
  }, [hasRisks]);
  ...
  const handleClick = (status: string) => {
    ...
    if (status === 'Estimate' && !hasRisks) {
      setEstimateClickError(ESTIMATE_CLICK_GATE);
      onNeedRisks?.();
      return;
    }
    setEstimateClickError('');
  ...
  return (
    <div className="mb-6">
              ...
              <StatusBox
                ...
                title={
                  isActive
                    ...
                    : clickable && status === 'Estimate' && !hasRisks
                    ? ESTIMATE_CLICK_GATE
                    : clickable
      ...
      {interactive && allowedNext.includes('Estimate') && !hasRisks && !estimateClickError && (
        <p className="text-xs text-amber-600 dark:text-amber-400 mt-2">
          Add at least one risk on the Risks tab before moving to Estimate.
        </p>
      )}
      {interactive && allowedNext.includes('Approved') && !allRisksClosed && (
        <p className="text-xs text-amber-600 dark:text-amber-400 mt-2">
          Record at least one risk and close all risks before approving.
        </p>
      )}
      {isTerminal ? (
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
          This use case is {currentStatus.toLowerCase()} and read-only. Review the completed record on this page.
        </p>
      ) : (
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
          Analysis covers Technical and Business tracks (auto-advances to Review when both complete and at least one risk exists).
