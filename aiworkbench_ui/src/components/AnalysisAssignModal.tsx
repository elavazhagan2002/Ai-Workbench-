...
export default function AnalysisAssignModal({
  ...
  useEffect(() => {
    ...
    setError('');
    setLoading(true);
    Promise.all([api.getEligibleTechnicalOwners(domainId), api.getEligibleBusinessOwners(domainId)])
      .then(([tech, biz]) => {
        setArchitects(tech || []);
        setReviewers(biz || []);
      })
  ...
  }, [open, domainId, initialTechnicalOwner, initialBusinessOwner, initialDueDate]);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!lockTechnicalOwner && !architects.some((u) => u.user_id === technicalOwner)) {
      setError('Select a technical owner who has access to this domain.');
      return;
    }
    if (!lockBusinessOwner && !reviewers.some((u) => u.user_id === businessOwner)) {
      setError('Select a business owner who has access to this domain.');
      return;
    }
    if (!dueDate) {
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400">
            {isPartialReassign
              ...
              : 'Select Technical Owner, Business Owner, and due date. Only users with access to this domain are listed.'}
          </p>
          {!loading && ((!lockTechnicalOwner && architects.length === 0) || (!lockBusinessOwner && reviewers.length === 0)) && (
            <div className="text-sm text-amber-800 dark:text-amber-200 bg-amber-50 dark:bg-amber-950/30 rounded-lg px-3 py-2">
              No eligible owners with domain access. Grant domain access to a tech_architect and business_reviewer first, then assign.
            </div>
          )}
          {error && (
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              ...
              disabled={
                saving
                || loading
                || (!lockTechnicalOwner && architects.length === 0)
                || (!lockBusinessOwner && reviewers.length === 0)
              }
