...
export function appendEstimateSection(
  ...
  if (isVendorBuild(data)) {
    yPos = appendSectionHeading(doc, 'Vendor Assessment', yPos, marginLeft);
    autoTable(doc, {
      startY: yPos,
      head: [['Question', 'Answer']],
      body: VENDOR_ASSESSMENT_QUESTIONS.map((question) => [
        question.question,
      ...
      columnStyles: { 0: { cellWidth: 140 }, 1: { cellWidth: 28 } },
      margin: { left: marginLeft, right: marginLeft, bottom: 16 },
    });
    yPos = getTableFinalY(doc, yPos) + 10;
  }
