...
export default function UseCaseEdit({
  ...
  const assessmentAssignPromptShown = useRef(false);
  ...
  const [hasDomainMembership, setHasDomainMembership] = useState(false);
  ...
  useEffect(() => {
    const isClosed = formData.status === 'Approved' || formData.status === 'Rejected';
    if (!currentDomainId || isClosed || (!hasCaseEdit && !canAssignOwners)) {
      setArchitects([]);
      setReviewers([]);
      return;
    }
    loadUsersByRole(currentDomainId);
  }, [currentDomainId, formData.status, hasCaseEdit, canAssignOwners]);
  ...
  useEffect(() => {
    ...
    assessmentAssignPromptShown.current = false;
  }, [useCaseId]);
  ...
  async function loadUseCase() {
    try {
      ...
      const formatOwnerLabel = (name?: string | null, email?: string | null) => {
        const displayName = (name || '').trim();
        if (!displayName) return '';
        const displayEmail = (email || '').trim();
        return displayEmail ? `${displayName} (${displayEmail})` : displayName;
      };
      setTechnicalOwnerLabel(formatOwnerLabel(data.technical_owner_name, data.technical_owner_email));
      setBusinessOwnerLabel(formatOwnerLabel(data.business_owner_name, data.business_owner_email));
      ...
      if (data.domain_id) {
        try {
          ...
          setDomainOwnerId((domainData as any)?.owner_id || null);
          // Successful getDomain means the user has DomainAccess or is owner/admin.
          setHasDomainMembership(true);
        } catch {
          setDomainName('');
          setDomainOwnerId(null);
          setHasDomainMembership(false);
        }
      } else {
        setDomainOwnerId(null);
        setHasDomainMembership(false);
  ...
  async function saveUseCaseChanges(options: {
    ...
    try {
      ...
      const canAssignAnalysisLocal =
        ...
        ((user?.role?.role_name === 'ai_leader' || user?.role?.role_name === 'domain_owner') &&
          hasDomainMembership);
  ...
  useEffect(() => {
    if (loading || !relatedLoaded || assessmentAssignPromptShown.current) return;
    if (activeSection !== 'assessment') return;
    if (!needsAssessmentOwnerAssignment) return;
    if (!canAssignAiAssessment) return;
    assessmentAssignPromptShown.current = true;
    setShowAssessmentAssignModal(true);
  }, [
    activeSection,
    canAssignAiAssessment,
    loading,
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Technical Owner (tech_architect)</label>
      ...
      {activeSection === 'assessment' && (
        <div className="space-y-4">
          ...
          <UseCaseAssessmentPanel
            ...
            needsAssessmentOwnerAssignment={needsAssessmentOwnerAssignment}
            canAssignAssessmentOwner={canAssignAiAssessment}
            onAssignAssessmentOwner={() => setShowAssessmentAssignModal(true)}
            canInitiateAssessment={canStartAssessmentChecklist}
            canContributeAssessment={canContributeAssessmentChecklist}
