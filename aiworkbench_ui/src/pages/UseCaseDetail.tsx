...
export default function UseCaseDetail({ useCaseId, onBack, initialTab = 'overview' }: UseCaseDetailProps) {
  ...
  const isAssessmentOwner = !!user?.user_id && useCase?.assessment_owner === user.user_id;
  const isClosedRecord = isClosedUseCaseStatus(useCase?.status);
  const needsAssessmentOwnerAssignment =
    useCase?.status === 'AI Assessment' &&
    !useCase?.assessment_owner &&
    !useCase?.assessment_completed_dt;
  const canInitiateAssessmentPermission =
    hasPermission('initiate_assessment');
  const canInitiateAssessment =
    canInitiateAssessmentPermission &&
    !isClosedRecord &&
    !needsAssessmentOwnerAssignment &&
    (isAdmin || isAssessmentOwner);
  const canContributeAssessment =
    (hasPermission('contribute_assessment') || canInitiateAssessmentPermission) &&
    !isClosedRecord &&
    !needsAssessmentOwnerAssignment &&
    (isAdmin || isAssessmentOwner);
