...
function AuthPanel({
  ...
  const title = titleOverride || panelCopy.title;
  const description = descriptionOverride || panelCopy.description;
    ...
    <section className={cn('relative order-1 min-w-0 overflow-hidden border-t border-slate-200/80 bg-[radial-gradient(circle_at_16%_14%,rgba(34,211,238,0.07),transparent_24%),radial-gradient(circle_at_84%_10%,rgba(59,130,246,0.06),transparent_24%),linear-gradient(180deg,#eff4fb_0%,#e7eef8_44%,#dde6f3_100%)] dark:border-white/10 xl:order-1 xl:h-[100dvh] xl:border-t-0 xl:bg-transparent dark:xl:bg-transparent', AUTH_LEFT_PANEL_DARK_BG_CLASS)}>
      ...
      <div className="mx-auto flex h-full w-full min-w-0 max-w-[560px] flex-col px-4 py-3 sm:px-5 sm:py-3.5 md:px-6 md:py-4 xl:max-w-none xl:px-6 xl:py-5 2xl:px-7">
        <div className="mx-auto flex h-full w-full min-w-0 max-w-[448px] flex-col lg:max-w-[500px] xl:max-w-[440px]">
          ...
          <div className={cn('flex flex-col', compact ? 'mt-1.5' : 'mt-2.5')}>
            <div className={cn(compact ? 'space-y-0.5' : 'min-h-[84px] space-y-1 sm:min-h-[88px]')}>
              <div>
                ...
                <h1 className={cn(
                  'font-semibold leading-[1.05] text-slate-950 dark:text-white',
                  compact ? 'mt-0.5 text-[1.2rem]' : 'mt-1 text-[clamp(1.25rem,1.1rem+0.45vw,1.65rem)]',
                )}>
                  {title}
                </h1>
                <p className={cn(
                  'max-w-xl text-slate-600 dark:text-slate-300',
                  compact ? 'mt-0.5 text-[11.5px] leading-[1.4]' : 'mt-0.5 text-[12px] leading-[1.45] sm:text-[12.5px]',
                )}>
                  {description}
                </p>
              </div>
            ...
            <div className={cn(
              'rounded-[22px] border border-slate-200/90 bg-[linear-gradient(180deg,rgba(255,255,255,0.9),rgba(248,250,252,0.96)_18%,rgba(241,245,249,0.94)_100%)] shadow-[inset_0_1px_0_rgba(255,255,255,0.86),0_22px_52px_-34px_rgba(148,163,184,0.45)] backdrop-blur-md dark:border-white/10 dark:bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(14,27,63,0.28)_18%,rgba(10,20,48,0.18)_100%)] dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_22px_54px_-32px_rgba(8,47,73,0.5)] dark:backdrop-blur-xl',
              compact ? 'mt-1.5 p-2.5' : 'mt-2 p-3',
            )}>
    ...
    <ViewportAuthShell
      authPanel={
        <AuthPanel
          ...
          compact={loginStep !== 'CREDENTIALS'}
          hideTabs={loginStep === 'MFA' || loginStep === 'TOTP_SETUP'}
          titleOverride={
          ...
          descriptionOverride={
            loginStep === 'TOTP_SETUP'
              ? 'Scan the QR, then enter the 6-digit code from Google Authenticator, Microsoft Authenticator, or Authy.'
              : loginStep === 'MFA'
                ? loginMfaMethod === 'totp'
                  ? 'Open your authenticator app and enter the current 6-digit code.'
                  : `We sent a 6-digit code to ${maskedLoginEmail}.`
                : undefined
          }
        >
          {activeView === 'login' ? (
            <div className="login-form flex flex-col gap-2">
              {loginStep === 'CREDENTIALS' ? (
                ...
                <form onSubmit={handleLoginMfaSubmit} className="flex flex-col gap-2.5">
                  ...
                  <div>
                    <label htmlFor="login-mfa-code" className="mb-1.5 block text-[11.5px] font-medium text-slate-700 dark:text-slate-200">
                      {loginMfaMethod === 'totp' ? '6-digit app code' : '6-digit email code'}
                    </label>
                    <OtpCodeField
                      id="login-mfa-code"
                      value={loginMfaCode}
                      inputRef={loginMfaInputRef}
                      disabled={isVerifyingLoginMfa || isSwitchingLoginMfaMethod}
                      onChange={handleLoginMfaCodeChange}
                    />
                  </div>
                  ...
                  {loginMfaMethod === 'email_passcode' ? (
                    <button
                      ...
                      className={`${subtleLinkClass} w-full justify-center`}
                    >
