  ...
  async signIn(email: string, password: string): Promise<SignInResponse> {
    ...
    if (data?.status === 'MFA_REQUIRED' && data?.challenge_id) {
      return {
        ...
        method: data.method || 'email_passcode',
...
async getPublicStats(): Promise<{ domains_count: number; use_cases_count: number }> {
  ...
  async getUseCaseDocuments(useCaseId: string) {
    return this.request<{ document_id: number; file_name: string; size_bytes?: number; uploaded_dt?: string }[]>(`/use-cases/${useCaseId}/documents`);
  }

  /** Upload reference documents. Uses multipart/form-data. */
  async uploadUseCaseDocuments(useCaseId: string, files: File[]) {
    const token = await this.getClientToken();
  ...
  async previewUseCaseDocument(
  useCaseId: string,
  documentId: number
): Promise<UseCaseDocumentPreview> {
  const token = await this.getClientToken();

  const response = await fetch(
    `${this.baseUrl}/use-cases/${useCaseId}/documents/${documentId}/preview`,
    {
      headers: {
        'Authorization': `Bearer ${token}`,
        Accept: [
  ...
  /** Users eligible for Technical Owner (tech_architect). */
  async getEligibleTechnicalOwners() {
    return this.request<{ user_id: string; user_name: string; user_email: string; role_name?: string }[]>('/use-cases/eligible-technical-owners');
  }

  /** Users eligible for Business Owner (business_reviewer). */
  async getEligibleBusinessOwners() {
    return this.request<{ user_id: string; user_name: string; user_email: string; role_name?: string }[]>('/use-cases/eligible-business-owners');
  }
  ...
  async getEligibleAssessmentOwners() {
    return this.request<{ user_id: string; user_name: string; user_email: string; role_name?: string }[]>(
      '/use-cases/eligible-assessment-owners'
    );
  }
