...
function contentWidthFor(lines: string[]): number {
  const longest = lines.reduce((best, line) => (line.length > best.length ? line : best), '');
  return Math.ceil(longest.length * 7.2 + 56);
}
...
function computePosition(
  ...
  const gap = 6;
  const pad = 8;
  const maxWidth = Math.max(pad * 2, vw - pad * 2);
  const width = Math.min(maxWidth, Math.max(rect.width, 180, minContentWidth));
  let left = rect.left;
...
export default function SelectMenu({
  ...
  const updatePosition = useCallback(() => {
    ...
    const minContentWidth = contentWidthFor(filtered.map((opt) => optionLine(opt, placeholderText)));
    setPos(computePosition(triggerRef.current, filtered.length, enableSearch, minContentWidth));
    ...
    open && pos && typeof document !== 'undefined'
      ? createPortal(
          <div
            ...
            className={cx(
              themeClass,
              'wb-select-menu flex min-w-0 flex-col overflow-hidden border border-line bg-surface-elevated text-ink shadow-lg',
                ...
                filtered.map((opt, index) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => choose(opt)}
                      className={cx(
                        'flex h-8 w-full items-center gap-2 px-2.5 pr-3 text-left text-[13px] leading-none text-ink transition',
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1 truncate">{optionLine(opt, placeholderText)}</span>
                    </button>
                  );
                })
              )}
