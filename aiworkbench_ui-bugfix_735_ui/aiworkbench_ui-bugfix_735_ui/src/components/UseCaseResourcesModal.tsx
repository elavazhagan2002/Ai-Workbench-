...
import type { UseCase, UseCaseDocument, UseCaseLink } from '../types';
...
type PreviewKind = 'pdf' | 'image' | 'html' | 'word' | 'excel' | 'powerpoint'  | 'unavailable';
...
function inferPreviewKind(
  ...
  if (normalizedType === 'DOC') return 'word';
  if (normalizedType === 'XLS') return 'excel';
  if (normalizedType === 'PPT') return 'powerpoint';
  ...
  if (
    normalizedMime.includes('application/vnd.ms-excel') ||
    normalizedMime.includes(
  ...
  if (
    normalizedMime.includes('application/vnd.ms-powerpoint') ||
    normalizedMime.includes(
...
export default function UseCaseResourcesModal({
  ...
  const documentItems = documents.filter((doc) => !parseInfographicFileName(doc.file_name).isInfographic);
  const infographicItems = documents.filter((doc) => parseInfographicFileName(doc.file_name).isInfographic);
  const documentUploadBlockedReason = null;
  ...
  async function loadDocuments() {
    ...
    try {
      ...
      setDocuments(list || []);
      setDocumentsLoaded(true);
      setResourceUseCase((prev) => ({ ...prev, documents: list || [] }));
    } catch (err: any) {
  ...
  async function handleOpenDocument(documentItem: UseCaseDocument) {
    ...
    try {
      ...
      // HTML preview stays in the existing iframe viewer.
      if (previewKind === 'html') {
      ...
      // Office files must be passed as the original Blob to the
      // browser-side renderer. Do not create an object URL for them.
      if (preview.blob) {
        if (previewKind === 'word') {
          setViewerPreview({
            kind: 'word',
        ...
        // PDF and image previews can use an object URL.
        if (previewKind === 'pdf' || previewKind === 'image') {
          ...
          setViewerPreview({
            kind: previewKind,
            src: objectUrl,
            ...(previewKind === 'pdf' ? { blob: preview.blob } : {}),
      ...
      // Base64 fallback. Convert it to a Blob before handing it to an
      // Office renderer, because the renderers work with binary files.
      if (preview.content_base64 && previewKind !== 'unavailable') {
        const mimeType = resolvePreviewMimeType(
          previewKind,
          preview.mime_type
        );
        const binaryString = window.atob(preview.content_base64);
      ...
      // URL fallback for PDF/image/HTML responses.
      if (preview.preview_url && previewKind !== 'unavailable') {
        if (previewKind === 'html') {
          ...
          setViewerPreview({
            kind: 'unavailable',
            message:
              'This Office document was returned without binary content required for browser preview.',
          });
        }
        return;
      }
            ...
            >
              ...
              {canEditResources && (
                <button
                  ...
                  disabled={documentActionKey !== null}
                  className="rounded p-1 text-slate-500 hover:text-red-600 disabled:opacity-50 dark:text-slate-400 dark:hover:text-red-400"
  ...
  return (
    <>
      <div className="fixed inset-0 z-40 flex items-start justify-center overflow-y-auto bg-black/70 px-4 pb-6 pt-20 sm:pt-24">
          ...
          {activeTab === 'documents' && (
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Uses the existing use case document upload, delete, and download APIs.
                </p>
              </div>
              ...
              {!canReadDocuments ? (
                ...
                <>
                  {canEditResources && (
                    <div className="space-y-2">
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        You can upload multiple document files here. Documents and infographics can both exist on the
                        same use case, but they must be uploaded in separate requests.
                      </p>
                      <label
                        className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm ${
                          documentUploadBlockedReason || documentUploading
                      ...
                      >
                        ...
                        <input
                          ...
                          disabled={documentUploading || !!documentUploadBlockedReason}
                          className="hidden"
                          onChange={(event) => {
