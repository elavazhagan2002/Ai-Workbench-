...
import type { UseCase, UseCaseData, UseCaseRiskReview, UseCaseComment, UseCaseLink, UseCaseDocument } from '../types';
import { ArrowLeft, Save, Plus, Trash2, X, ChevronRight, ArrowRightLeft, Link as LinkIcon, Paperclip, Edit2, HelpCircle } from 'lucide-react';
...
import UseCaseStateView, { STATUS_TO_WORKFLOW_PERMISSION, WORKFLOW_NEXT } from '../components/UseCaseStateView';
...
export default function UseCaseEdit({ useCaseId, onBack }: UseCaseEditProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    expected_benefits: '',
    department: '',
    ai_category: DEFAULT_AI_CATEGORY,
  ...
  // Default active section by stage
  useEffect(() => {
    const s = formData.status;
    if (s === 'New') setActiveSection('basic');
    else if (s === 'Analysis') setActiveSection('tech_analysis');
    else if (s === 'Review') setActiveSection('tech_analysis');
    else if (s === 'Estimate') setActiveSection('estimate');
    else if (s === 'ROI') setActiveSection('roi');
    else if (s === 'AI Assessment') setActiveSection('assessment');
    else if (s === 'Approved' || s === 'Rejected') setActiveSection('comments');
  ...
  async function loadUsersByRole() {
    try {
      const [architectsData, reviewersData, estimateOpts] = await Promise.all([
        api.getEligibleTechnicalOwners(),
        api.getEligibleBusinessOwners(),
  ...
  async function loadUseCase() {
    try {
      ...
      setEstimateMeta({
        ...
        estimate_data: (data as any).estimate_data
          ? normalizeEstimateData((data as any).estimate_data)
          : null,
      });
      const inv =
        (data as any).estimate_data?.totals
          ? Number((data as any).estimate_data.totals.year1 || 0) +
            Number((data as any).estimate_data.totals.year2 || 0) +
            Number((data as any).estimate_data.totals.year3 || 0)
          : 0;
      setRoiMeta({
        ...
        roi_data: (data as any).roi_data
          ? normalizeRoiData((data as any).roi_data, defaultCurrency, inv)
          : null,
      });
      setAssessmentMeta({
  ...
  const estimateInvestmentTotal = estimateMeta.estimate_data?.totals
    ? Number(estimateMeta.estimate_data.totals.year1 || 0) +
      Number(estimateMeta.estimate_data.totals.year2 || 0) +
      Number(estimateMeta.estimate_data.totals.year3 || 0)
    : 0;
  ...
  const isReadOnly = (formData.status === 'Approved' || formData.status === 'Rejected') || !canEditCase;
    ...
    <div className="wb-page">
      <div>
        ...
        <p className="wb-page-subtitle mb-4">
          {formData.status === 'Approved' || formData.status === 'Rejected'
            ? 'Use case is approved/rejected. You can still add comments and ratings.'
      ...
      <div className="wb-card-pad">
        ...
        {(formData.status === 'Analysis' || formData.status === 'Review' || formData.status === 'Estimate') && (
          <div className="mt-4 space-y-2 text-sm text-slate-600 dark:text-slate-400">
            ...
            <p>
              Technical: {formData.technical_owner ? (technicalOwnerLabel || 'Assigned') : 'Unassigned'}
              {analysisMeta.tech_analysis_completed_dt ? ` · Completed ${new Date(analysisMeta.tech_analysis_completed_dt).toLocaleString()}` : ''}
              {analysisMeta.tech_analysis_rejected_dt ? ` · Rejected: ${analysisMeta.tech_analysis_rejection_note || ''}` : ''}
            </p>
            <p>
              Business: {formData.business_owner ? (businessOwnerLabel || 'Assigned') : 'Unassigned'}
              {analysisMeta.business_analysis_completed_dt ? ` · Completed ${new Date(analysisMeta.business_analysis_completed_dt).toLocaleString()}` : ''}
              {analysisMeta.business_analysis_rejected_dt ? ` · Rejected: ${analysisMeta.business_analysis_rejection_note || ''}` : ''}
            </p>
            ...
            {formData.status === 'Estimate' && canAssignEstimate && (
              <button
                type="button"
                onClick={() => setShowEstimateAssignModal(true)}
          ...
          {(() => {
            ...
            const order: Section[] =
              status === 'New'
                ...
                : status === 'ROI'
                ? ['roi', 'estimate', 'basic', 'references', 'data', 'risks', 'comments', 'audit']
                : status === 'AI Assessment'
                ? ['assessment', 'roi', 'risks', 'basic', 'references', 'data', 'comments', 'audit']
                : status === 'Review'
                ? ['basic', 'tech_analysis', 'business_analysis', 'risks', 'references', 'data', 'comments', 'audit']
                : status === 'Approved' || status === 'Rejected'
                ? ['risks', 'basic', 'references', 'data', 'comments', 'audit']
                : ['basic', 'references', 'audit'];
            const labels: Record<Section, string> = {
              basic: 'Basic Info',
              references: `References (${(formData.reference_links?.length || 0) + documents.length})`,
              analysis: 'Analysis',
              tech_analysis: 'Technical Analysis',
              business_analysis: 'Business Analysis',
              data: `Data Requirements (${dataReqs.length})`,
              risks: `Risks (${riskReviews.length})`,
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
          ...
          <div className="space-y-8">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">Attach files (stored on server; path configurable via FILE_STORAGE_ROOT).</p>
              {!isReadOnly && (
                <div className="mb-3">
                  <label className="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-700 rounded-lg cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-600">
                    <Plus className="w-4 h-4" />
                    <span>{documentUploading ? 'Uploading...' : 'Choose files to upload'}</span>
                    <input
                      type="file"
                      multiple
              ...
              <ul className="space-y-2">
                {documents.map((doc) => (
                  <li key={doc.document_id} className="flex items-center gap-2 flex-wrap">
                    <span className="text-slate-700 dark:text-slate-300 truncate">{doc.file_name}</span>
                    {doc.size_bytes != null && <span className="text-slate-400 text-sm">({(doc.size_bytes / 1024).toFixed(1)} KB)</span>}
                    <button
                      type="button"
                      onClick={async () => {
                    ...
                    {!isReadOnly && (
                      <button type="button" onClick={() => handleDeleteDocument(doc.document_id)} className="p-1 text-slate-500 hover:text-red-600 rounded" title="Delete document">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    )}
                  </li>
                ...
                {documents.length === 0 && (
                  <li className="text-slate-500 dark:text-slate-400 text-sm italic">No documents attached yet.</li>
                )}
              </ul>
            </div>
          </div>
      ...
      {activeSection === 'tech_analysis' && (
        <AnalysisPanels
          ...
          canComplete={(isTechOwner || isAdmin) && !analysisMeta.tech_analysis_completed_dt}
      ...
      {activeSection === 'business_analysis' && (
        <AnalysisPanels
          ...
          canComplete={(isBizOwner || isAdmin) && !analysisMeta.business_analysis_completed_dt}
      ...
      {activeSection === 'risks' && (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
          ...
          {!isReadOnly && canReviewRisk && showRiskForm && (
          ...
          <div className="space-y-3">
            {riskReviews.length === 0 ? (
              ...
              riskReviews.map((risk) => {
                const canUpdateReview = canReviewRisk && (risk.assigned_to === user?.user_id || isAdmin) && (editingRiskReviewId === risk.risk_review_id || !editingRiskReviewId);
      ...
      {activeSection === 'roi' && (
        <div className="space-y-4">
          ...
          <RoiPanel
            ...
            investmentByYear={{
              year1: Number(estimateMeta.estimate_data?.totals?.year1 || 0),
              year2: Number(estimateMeta.estimate_data?.totals?.year2 || 0),
              year3: Number(estimateMeta.estimate_data?.totals?.year3 || 0),
            }}
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                  Assign a governance owner, run AI pre-fill, validate responses, then close the checklist and complete the assignment.
                  Final Approve/Reject is available after assessment completion (all risks must be closed for Approve).
                </p>
          ...
          <UseCaseAssessmentPanel
            ...
            canInitiateAssessment={
              canInitiateAssessment &&
              formData.status === 'AI Assessment' &&
              (isAdmin || isAssessmentOwner || !assessmentMeta.assessment_owner)
            }
          ...
          <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4 space-y-2">
            ...
            <p className="text-sm text-amber-800 dark:text-amber-300">
              List of controls and validation deliverables to be created — pending discussion with Validation Expert (Sachin).
            </p>
            {isVendorBuild(estimateMeta.estimate_data) ? (
              <>
                ...
                <p className="text-sm text-amber-800 dark:text-amber-300">
                  Build Cost is Vendor. Complete / review the vendor evaluation questions on the Estimate tab.
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-sm text-amber-800 dark:text-amber-300">
                  Shown when Build Cost type is Vendor on the Estimate tab.
                </p>
              </>
            )}
          </div>
      ...
      {activeSection === 'comments' && (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">
            {canComment ? 'Add comments and optional 1–5 star rating. Visible in use case edit and detail.' : 'Comments and ratings. You need case_comment permission to add or update.'}
          </p>
          {canComment && (
              ...
              comments.map((c) => (
                <div key={c.comment_id} className="flex items-start justify-between p-4 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50">
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    {user?.user_id === c.comment_by && (canComment && editingCommentRating === c.comment_id ? (
