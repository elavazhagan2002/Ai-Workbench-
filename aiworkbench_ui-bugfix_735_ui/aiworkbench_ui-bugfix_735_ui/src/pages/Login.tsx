...
import { api, type PasswordValidationResult, type UsernameAvailabilityResult } from '../lib/api';
...
export default function Login() {
  ...
  const { signIn, verifyLoginPasscode } = useAuth();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (response.status === 'MFA_REQUIRED') {
        ...
        setLoginMfaCode('');
        setLoginMfaMessage({ type: 'info', text: 'Enter the verification code sent to your email' });
  ...
  async function handleLoginMfaVerify(codeOverride?: string) {
    ...
    try {
      await verifyLoginPasscode(loginChallengeId, passcode);
  ...
  async function handleResendLoginMfaCode() {
    if (!loginChallengeId || loginMfaResendCooldown > 0 || isResendingLoginMfa) return;
            ...
            <div className="login-form flex flex-col gap-2">
              {loginStep === 'CREDENTIALS' ? (
                ...
                <form onSubmit={handleLoginMfaSubmit} className="flex flex-col gap-2.5">
                  ...
                  <button type="submit" disabled={!canSubmitLoginMfa} className={PRIMARY_BUTTON_CLASS}>
                    <Lock className="h-4 w-4" />
