...
export default function UseCases({ domainId, onBack, onViewUseCase, onEditUseCase, selectedStatus, onStatusChange }: UseCasesProps) {
  ...
  function renderDocumentationScoreTrigger(useCase: UseCase) {
    ...
    const label = isPending ? 'Updating...' : getDocumentationScoreLabel(summary);
    ...
    return (
      <div className="group relative inline-flex shrink-0">
        <button
          type="button"
          onClick={(event) => handleDocumentationQualityTrigger(useCase, event)}
          disabled={isPending}
        ...
        {summary && !isPending && (
          <div className="pointer-events-none invisible absolute right-0 top-full z-20 mt-2 w-64 rounded-xl border border-slate-200 bg-white/95 p-3 text-left opacity-0 shadow-xl transition duration-150 group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100 dark:border-slate-700 dark:bg-slate-900/95">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className={`rounded-full border px-2 py-1 text-[11px] font-semibold ${scoreTone}`}>
    ...
    try {
      const exportData = await Promise.all(
        useCases.map(async (uc) => {
          const [fullUseCase, dataReqs, riskReviews, comments] = await Promise.all([
      ...
      doc.setFontSize(9);
      doc.setTextColor(100, 116, 139);
      doc.text(`Report generated on ${currentDate}. The following pages list each use case with full analysis, data requirements, risks, and comments.`, marginLeft, yPos);
      doc.setTextColor(0, 0, 0);
      yPos += 20;

      // Use case pages
      for (let i = 0; i < exportData.length; i++) {
        const { useCase: uc, dataReqs, riskReviews, comments } = exportData[i];
        doc.addPage();
        yPos = 20;
        ...
        const statusColors: Record<string, [number, number, number]> = {
          'Production': [34, 197, 94], 'Approved': [59, 130, 246], 'Development': [6, 182, 212], 'Testing': [168, 85, 247]
        };
        const statusColor = statusColors[uc.status] || [100, 116, 139];
        doc.setFillColor(statusColor[0], statusColor[1], statusColor[2]);
        doc.setTextColor(255, 255, 255);
        doc.setFontSize(9);
        const statusText = uc.status;
        const statusW = doc.getTextWidth(statusText) + 6;
        doc.roundedRect(marginLeft, yPos - 3, statusW, 5, 2, 2, 'F');
        doc.text(statusText, marginLeft + 3, yPos + 1);
        doc.setTextColor(0, 0, 0);
        yPos += 10;
        ...
        doc.setFontSize(10);
        if (uc.use_case_description) {
          yPos = appendTextSection(doc, 'Description', uc.use_case_description, yPos, pageWidth, marginLeft);
        }
        if (uc.solution_design_overview) {
          yPos = appendTextSection(doc, 'Solution Design Overview', uc.solution_design_overview, yPos, pageWidth, marginLeft);
        }

        yPos = appendInfoTable(doc, buildUseCaseInfoRows(uc), yPos, marginLeft);
              ...
              {currentUseCases.map((useCase) => (
                <tr key={useCase.use_case_id} className="hover:bg-surface-muted/70">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">
                    {useCase.created_by_name ?? 'â€”'}
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-900 dark:text-white">
                    {useCase.ai_category ? getAiCategoryLabel(useCase.ai_category) : '-'}
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-900 dark:text-white">{useCase.department || '-'}</td>
                  <td className="px-6 py-4">
                    {useCase.average_rating != null ? (
                      <div className="flex items-center gap-2">
                        <StarRating rating={useCase.average_rating} size="sm" />
