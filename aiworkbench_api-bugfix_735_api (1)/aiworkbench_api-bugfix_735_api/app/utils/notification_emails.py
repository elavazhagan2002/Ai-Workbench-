...
def notify_admins_new_use_case(
    ...
    subject = "New Use Case Created in AI Governance Workbench"
    body_text = f"""Hello Admin,

A new use case has been created in AI Governance Workbench.

Use case name: {getattr(use_case, "use_case_name", "")}
Use case title: {getattr(use_case, "use_case_title", None) or "Not provided"}
Domain: {getattr(domain, "domain_name", None) or getattr(use_case, "domain_id", "")}
Status: {getattr(use_case, "status", "")}
Created by: {_display_name(created_by)}
Created date: {_format_dt(getattr(use_case, "created_dt", None))}

Please review the use case if governance follow-up, assignment, or prioritization is needed.
...
def notify_analysis_assignment(
    ...
    *,
    ...
    for owner, track in ((technical_owner, "Technical"), (business_owner, "Business")):
...
def notify_analysis_rejection(
    ...
    *,
    ...
    """Notify the assigner that a track assignment was rejected."""
...
def notify_analysis_completed(
    ...
    *,
    ...
    """Notify the assigner that a track analysis was marked completed."""
    if not assigner or not _is_usable_email(assigner.user_email):
        return
    title = getattr(use_case, "use_case_title", None) or use_case.use_case_name
...
def notify_estimate_completed(
    ...
    *,
    ...
    """Notify the assigner that Estimate was completed and moved to ROI."""
    if not assigner or not _is_usable_email(assigner.user_email):
        return
    title = getattr(use_case, "use_case_title", None) or use_case.use_case_name
...
def notify_roi_previous_owner(
    ...
    *,
    ...
    if not previous_owner or not _is_usable_email(previous_owner.user_email):
        return
...
def notify_assessment_completed(
    ...
    *,
    ...
    if not assigner or not _is_usable_email(assigner.user_email):
        return
    title = getattr(use_case, "use_case_title", None) or use_case.use_case_name
    subject = f"[AI Workbench] AI Assessment completed: {title}"
    body_text = f"""Hello {_display_name(assigner)},
