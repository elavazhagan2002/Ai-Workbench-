...
async def create_or_get_assessment(
    ...
    if use_case.status != "AI Assessment":
        from app.core.authorization import get_user_with_permissions, is_admin

        user, _, is_user_admin = get_user_with_permissions(db, user_id)
        if not is_user_admin:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Responsible AI assessment can only be initiated when the use case is in AI Assessment status.",
            )
