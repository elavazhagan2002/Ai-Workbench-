...
class SignInMfaRequiredResponse(BaseModel):
    ...
    message: str
    challenge_id: str
    method: Literal["email_passcode"] = "email_passcode"
...
def _complete_signin_success(
    ...
    return SignInSuccessResponse(user=user_response, message="Sign-in successful")
...
async def signin(
    ...
    challenge_issue = login_mfa.create_login_challenge(user.user_id, user.user_email, ip_address)
    if challenge_issue.status == "too_many_resends":
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail="Too many login verification passcode requests. Please try again later.",
        )
    if challenge_issue.status == "unavailable":
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Login verification service is temporarily unavailable. Please try again in a moment.",
        )
    ...
    if challenge_issue.status == "issued":
        _add_auth_audit(
            db,
            user.user_id,
...
async def verify_login_passcode(
    ...
    if verification.status != "verified":
        ...
        _add_auth_audit(db, user_id or None, "login_mfa_failed", failure_details)
        db.commit()
...
async def resend_login_passcode(
    ...
    resend = login_mfa.resend_login_challenge(body.challenge_id)
