...
def _parse_due_date(value: str):
    ...
    try:
        if len(raw) == 10:
            return dt_cls.strptime(raw, "%Y-%m-%d")
        return dt_cls.fromisoformat(raw.replace("Z", "+00:00")).replace(tzinfo=None)
...
def _can_assign_analysis(db: Session, user: User, domain_id: str) -> bool:
    ...
    return check_permission(
        db, user, "workflow_analysis", allow_admin=True, domain_id=domain_id
    ) and check_permission(db, user, "case_assign", allow_admin=True, domain_id=domain_id)
...
def _can_review_use_case(db: Session, user: User, domain_id: str) -> bool:
    ...
    if is_domain_owner(db, user, domain_id):
        return True
    return check_permission(db, user, "workflow_review", allow_admin=True, domain_id=domain_id)
...
def _can_assign_estimate(db: Session, user: User, domain_id: str) -> bool:
    ...
    if is_domain_owner(db, user, domain_id):
        return True
    return check_permission(
        db, user, "workflow_estimate", allow_admin=True, domain_id=domain_id
    ) and check_permission(db, user, "case_assign", allow_admin=True, domain_id=domain_id)
...
def _can_assign_roi(db: Session, user: User, domain_id: str) -> bool:
    ...
    if is_domain_owner(db, user, domain_id):
        return True
    return check_permission(
        db, user, "workflow_roi", allow_admin=True, domain_id=domain_id
    ) and check_permission(db, user, "case_assign", allow_admin=True, domain_id=domain_id)
...
def _can_assign_ai_assessment(db: Session, user: User, domain_id: str) -> bool:
    ...
    if is_domain_owner(db, user, domain_id):
        return True
    return check_permission(
        db, user, "workflow_ai_assessment", allow_admin=True, domain_id=domain_id
    ) and check_permission(db, user, "case_assign", allow_admin=True, domain_id=domain_id)
...
def _has_assessment_participation_permission(db: Session, user: User) -> bool:
    return (
        ...
        or check_permission(db, user, "case_assess", allow_admin=True)
    )
...
async def get_eligible_technical_owners(
    ...
    """Get tech_architect users eligible as Technical Owner for Analysis assignment."""
    ...
    users = _get_users_by_role_names(db, ANALYSIS_TECHNICAL_OWNER_ROLES)
    return [
...
async def get_eligible_business_owners(
    ...
    """Get business_reviewer users eligible as Business Owner for Analysis assignment."""
    user_id = get_current_user_id(request)
    user, _, is_user_admin = get_user_with_permissions(db, user_id)
    if not is_user_admin and not check_permission(db, user, "case_edit", allow_admin=False) and not check_permission(db, user, "case_assign", allow_admin=False):
    ...
    users = _get_users_by_role_names(db, ANALYSIS_BUSINESS_OWNER_ROLES)
    return [
        EligibleOwnerResponse(
            user_id=u.user_id,
            user_name=u.user_name,
...
async def get_eligible_roi_owners(request: Request, db: Session = Depends(get_db)):
    """Users eligible to own ROI worksheets."""
    user_id = get_current_user_id(request)
    user, _, is_user_admin = get_user_with_permissions(db, user_id)
    ...
    users = _get_users_by_role_names(db, ROI_OWNER_ROLES)
    return [
        EligibleOwnerResponse(
            user_id=u.user_id,
            user_name=u.user_name,
...
async def get_eligible_assessment_owners(request: Request, db: Session = Depends(get_db)):
    """Governance-team users eligible to own AI Assessment."""
    user_id = get_current_user_id(request)
    user, _, is_user_admin = get_user_with_permissions(db, user_id)
    if not is_user_admin and not check_permission(db, user, "case_assign", allow_admin=False):
        require_permission(db, user, "case_view", allow_admin=True)
    users = _get_users_by_role_names(db, ASSESSMENT_OWNER_ROLES)
    return [
        EligibleOwnerResponse(
            user_id=u.user_id,
            user_name=u.user_name,
    ...
    if not is_user_admin and not is_owner:
        ...
        if not access:
            ...
            raise HTTPException(
                ...
                detail="No access to this domain"
            )
...
def _get_use_case_resource_file_counts(db: Session, use_case_id: str) -> dict[str, int]:
    ...
    infographic_count = sum(1 for doc in docs if _is_infographic_file_name(doc.file_name))
...
class _SafeHTMLPreviewSanitizer(HTMLParser):
    """Minimal HTML sanitizer for internal preview rendering."""

    allowed_tags = {
        "a", "article", "b", "blockquote", "br", "code", "div", "em", "figcaption",
        "figure", "h1", "h2", "h3", "h4", "h5", "h6", "hr", "img", "li", "main",
        "ol", "p", "pre", "section", "span", "strong", "style", "table", "tbody",
        "td", "th", "thead", "tr", "u", "ul"
    }
    ...
    def _is_attr_allowed(self, tag: str, attr_name: str) -> bool:
        return attr_name in self.allowed_attrs.get("*", set()) or attr_name in self.allowed_attrs.get(tag, set())
    ...
    def handle_starttag(self, tag: str, attrs):
        ...
        for attr_name, attr_value in attrs:
            attr_name = (attr_name or "").lower()
            if attr_name.startswith("on") or not self._is_attr_allowed(tag, attr_name):
                continue
...
def _sanitize_html_for_preview(content: bytes) -> str:
    """Return sanitized HTML wrapped in a minimal preview document."""
    sanitizer = _SafeHTMLPreviewSanitizer()
    sanitizer.feed(_decode_text_content(content))
    sanitizer.close()
    body = sanitizer.get_html().strip() or "<p>No previewable HTML content found.</p>"
    return (
        "<!DOCTYPE html>"
        "<html><head><meta charset=\"utf-8\">"
        "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">"
        "<style>"
        "body{font-family:Segoe UI,Tahoma,sans-serif;margin:0;padding:24px;line-height:1.5;color:#1f2937;background:#fff;}"
        "img{max-width:100%;height:auto;}table{border-collapse:collapse;width:100%;}"
        "th,td{border:1px solid #d1d5db;padding:8px;text-align:left;}a{color:#1d4ed8;}"
        "</style></head><body>"
        f"{body}"
        "</body></html>"
    )
...
async def upload_use_case_documents(
    ...
    for doc in uploaded_docs:
        ...
        uploaded.append({"document_id": doc.document_id, "file_name": doc.file_name, "size_bytes": doc.size_bytes, "uploaded_dt": doc.uploaded_dt.isoformat() if doc.uploaded_dt else None})
    return uploaded
...
async def preview_use_case_document(
    ...
    user, _ = _require_use_case_access(
        db,
        user_id,
        use_case_id,
    )

    doc = _get_use_case_document_or_404(
        db,
        use_case_id,
        document_id,
    )

    _require_use_case_document_view_permission(
        db,
        user,
        doc,
    )
    ...
    # PDF / IMAGE
    # ---------------------------------------------------------
    # These file types can be displayed directly by the browser.
    ...
    # HTML
    # ---------------------------------------------------------
    # HTML is sanitized before being displayed.

    if category == "HTML":
        return HTMLResponse(
            content=_sanitize_html_for_preview(content),
            headers={
                "Content-Disposition": (
                    f'inline; filename="{safe_name}"'
                ),
    ...
    # OFFICE DOCUMENTS
    # ---------------------------------------------------------
    # Return the original Office file to the browser.
    # The frontend renders DOC/DOCX, XLS/XLSX and PPT/PPTX
    # with browser-side preview libraries.

    if category in {"DOC", "XLS", "PPT"}:
        return FileResponse(
            path=str(file_path),
            media_type=normalized_content_type,
            headers={
    ...
    # OTHER FILE TYPES
    # ---------------------------------------------------------
    # ZIP, RAR, TAR and other unsupported types do not have
    # an in-app preview.
...
async def update_use_case(
    ...
    # Once Approved or Rejected: no further updates to use case details; only portal_admin can override status
    if current_status in ("Approved", "Rejected"):
        if not is_user_admin:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Use case is approved/rejected. No further updates allowed. Only portal_admin can override status."
            )
        if is_status_change:
            if new_status not in VALID_STATUSES:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid status: {new_status}")
            if new_status == "Rejected" and not (use_case_data.rejection_reason or "").strip():
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
            ...
            if new_status == "Rejected":
                use_case.rejection_reason = (use_case_data.rejection_reason or "").strip() or None
            else:
                use_case.rejection_reason = None
        if use_case_data.reference_links is not None:
            _replace_use_case_reference_links(
                db=db,
                use_case_id=use_case_id,
                user_id=user_id,
                reference_links=use_case_data.reference_links,
            )
        use_case.modified_by = user_id
        db.commit()
        db.refresh(use_case)
        ...
        audit_log = AuditLog(
            ...
            action="update",
            user_id=user_id,
            details={"use_case_id": use_case_id, "use_case_name": use_case.use_case_name, "domain_id": use_case.domain_id, "status_override": new_status}
        )
        db.add(audit_log)
        db.commit()
        technical_owner_display = _resolve_user_display(db, use_case.technical_owner)
    ...
    # When transitioning to Approved, all risk reviews must be closed
    ...
    if is_status_change and new_status == "Estimate" and current_status == "Review" and not is_user_admin:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if is_status_change and new_status == "ROI" and current_status == "Estimate" and not is_user_admin:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if is_status_change and new_status == "AI Assessment" and current_status == "ROI" and not is_user_admin:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="AI Assessment is entered automatically when the ROI assignment is completed.",
        )

    # Approve/Reject only from AI Assessment for non-admin (Reject still allowed earlier via FORWARD_NEXT)
    if is_status_change and new_status == "Approved" and current_status != "AI Assessment" and not is_user_admin:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Use cases can only be Approved from AI Assessment status.",
        )

    # Block direct New → Analysis without assignment endpoint (unless portal_admin override with owners already set)
    if is_status_change and new_status == "Analysis" and current_status == "New" and not is_user_admin:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Use POST /use-cases/{id}/analysis/assign to move to Analysis with owners and due date.",
        )

    # Non-portal_admin: only allow forward workflow transitions
    ...
    if current_status == "Analysis" and not is_user_admin and not is_owner:
        ...
        # Completed track becomes read-only for that assignee
        if is_tech and getattr(use_case, "tech_analysis_completed_dt", None) and content_keys & TECHNICAL_ANALYSIS_FIELDS:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Technical analysis is already marked completed.",
            )
        if is_biz and getattr(use_case, "business_analysis_completed_dt", None) and content_keys & BUSINESS_ANALYSIS_FIELDS:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Business analysis is already marked completed.",
            )
...
async def assign_analysis(
    ...
    if not _can_assign_analysis(db, user, use_case.domain_id):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only portal_admin, domain owner, or users with workflow_analysis + case_assign can assign Analysis.",
        )
...
async def complete_analysis_track(
    ...
    if track == "technical":
        if not is_user_admin and use_case.technical_owner != user_id:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only the Technical Owner can complete technical analysis.")
        ...
        if not is_user_admin and use_case.business_owner != user_id:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only the Business Owner can complete business analysis.")
    ...
    notify_analysis_completed(
        db, use_case, track=label, completer=user, assigner=assigner, background_tasks=background_tasks
    )
...
async def send_back_analysis_track(
    ...
    if not _can_review_use_case(db, user, use_case.domain_id):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You do not have permission to send analysis back from Review (requires workflow_review).",
        )
...
def _estimate_is_complete(data: dict) -> tuple[bool, str | None]:
    ...
    for key in ESTIMATE_LINE_KEYS:
        ...
        if not line.get("type"):
            return False, f"{ESTIMATE_LINE_LABELS.get(key, key)} type is required."
        has_cost = any(line.get(y) is not None for y in ("year1", "year2", "year3"))
        if not has_cost:
            return False, f"{ESTIMATE_LINE_LABELS.get(key, key)} needs at least one year cost."
    return True, None
...
async def assign_estimate(
    ...
    if not _can_assign_estimate(db, user, use_case.domain_id):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only portal_admin, domain owner, or users with workflow_estimate + case_assign can assign Estimate.",
        )
...
async def complete_estimate(
    ...
    if not isinstance(getattr(use_case, "roi_data", None), dict):
        use_case.roi_data = empty_roi_payload(currency)
    else:
...
async def assign_roi(
    ...
    if not use_case:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Use case not found")
    if not _can_assign_roi(db, user, use_case.domain_id):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only portal_admin, domain owner, or users with workflow_roi + case_assign can assign ROI.",
        )
...
async def save_roi(
    ...
    user_currency = (payload.roi_data or {}).get("currency") if isinstance(payload.roi_data, dict) else None
    if isinstance(user_currency, str) and user_currency.strip().upper() in CURRENCY_OPTIONS:
        normalized["currency"] = user_currency.strip().upper()
...
async def complete_roi(
    ...
    if not use_case.roi_owner and not is_user_admin:
...
async def assign_ai_assessment(
    ...
    if not use_case:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Use case not found")
    if not _can_assign_ai_assessment(db, user, use_case.domain_id):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only portal_admin, domain owner, or users with workflow_ai_assessment + case_assign can assign AI Assessment.",
        )
...
async def complete_ai_assessment_assignment(
    ...
    if not use_case.assessment_owner and not is_user_admin:
