...
class _MemoryRedisClient:
    ...
    def setex(self, key, ttl_seconds, value):
        self._data[key] = {
            "value": value,
            "expires_at": time.time() + max(int(ttl_seconds), 0),
        }
        return True
    ...
    def expire(self, key, ttl_seconds):
        ...
        if entry is None:
            return False
        entry["expires_at"] = time.time() + max(int(ttl_seconds), 0)
        return True
...
try:
    real_redis_client.ping()
    redis_client = real_redis_client
    logger.info("Redis connection successful (email OTP)")
except Exception as e:
    redis_client = _MemoryRedisClient()
    logger.warning(f"Redis connection failed (email OTP), using in-memory fallback: {e}")
