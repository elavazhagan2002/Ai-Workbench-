...
class Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_file=[str(path) for path in _ENV_FILE_CANDIDATES if path.exists()],
    ...
    def validate_llm_configuration(self):
        """Fail fast when AI enhancement is enabled but the provider configuration is incomplete."""
        if not self.LLM_ENABLED:
            return self

        normalized_provider = (self.LLM_PROVIDER or "").strip().lower().replace(" ", "_")
        if not normalized_provider:
            inferred_base_url = (self.LLM_BASE_URL or "").strip().lower()
            if "ollama" in inferred_base_url or "11434" in inferred_base_url:
                normalized_provider = "ollama"

        missing = []
        if normalized_provider != "ollama" and not self.LLM_API_KEY:
            missing.append("LLM_API_KEY")
        if not self.LLM_MODEL:
            missing.append("LLM_MODEL")
        if not self.LLM_BASE_URL:
            missing.append("LLM_BASE_URL")
        if normalized_provider in {"azure", "azure_openai"} and not (self.LLM_AZURE_DEPLOYMENT or self.LLM_MODEL):
            missing.append("LLM_AZURE_DEPLOYMENT")

        if missing:
            raise ValueError(
                "LLM is enabled but required settings are missing: "
                + ", ".join(missing)
                + "."
            )
        return self
